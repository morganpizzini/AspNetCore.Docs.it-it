...
<span data-ttu-id="5a372-105">In questa sezione la logica di convalida viene aggiunta al modello `Movie`.</span><span class="sxs-lookup"><span data-stu-id="5a372-105">In this section, validation logic is added to the `Movie` model.</span></span> <span data-ttu-id="5a372-106">Le regole di convalida vengono applicate ogni volta che un utente crea o modifica un film.</span><span class="sxs-lookup"><span data-stu-id="5a372-106">The validation rules are enforced any time a user creates or edits a movie.</span></span>
...
## <a name="add-validation-rules-to-the-movie-model"></a><span data-ttu-id="5a372-116">Aggiungere regole di convalida al modello movie</span><span class="sxs-lookup"><span data-stu-id="5a372-116">Add validation rules to the movie model</span></span>

<span data-ttu-id="5a372-117">Lo `DataAnnotations` spazio dei nomi fornisce:</span><span class="sxs-lookup"><span data-stu-id="5a372-117">The `DataAnnotations` namespace provides:</span></span>

* <span data-ttu-id="5a372-118">Set di attributi di convalida predefiniti che vengono applicati in modo dichiarativo a una classe o a una proprietà.</span><span class="sxs-lookup"><span data-stu-id="5a372-118">A set of built-in validation attributes that are applied declaratively to a class or property.</span></span>
* <span data-ttu-id="5a372-119">Formattazione di attributi come `[DataType]` questa guida alla formattazione e non forniscono alcuna convalida.</span><span class="sxs-lookup"><span data-stu-id="5a372-119">Formatting attributes like `[DataType]` that help with formatting and don't provide any validation.</span></span>
...
* <span data-ttu-id="5a372-122">Gli attributi `[Required]` e `[MinimumLength]` indicano che una proprietà deve avere un valore.</span><span class="sxs-lookup"><span data-stu-id="5a372-122">The `[Required]` and `[MinimumLength]` attributes indicate that a property must have a value.</span></span> <span data-ttu-id="5a372-123">Niente impedisce a un utente di immettere spazi vuoti per soddisfare la convalida.</span><span class="sxs-lookup"><span data-stu-id="5a372-123">Nothing prevents a user from entering white space to satisfy this validation.</span></span>
* <span data-ttu-id="5a372-124">L'attributo `[RegularExpression]` viene usato per limitare i caratteri che possono essere inseriti.</span><span class="sxs-lookup"><span data-stu-id="5a372-124">The `[RegularExpression]` attribute is used to limit what characters can be input.</span></span> <span data-ttu-id="5a372-125">Nel codice precedente, `Genre` :</span><span class="sxs-lookup"><span data-stu-id="5a372-125">In the preceding code, `Genre`:</span></span>

  * <span data-ttu-id="5a372-126">Deve includere solo lettere.</span><span class="sxs-lookup"><span data-stu-id="5a372-126">Must only use letters.</span></span>
  * <span data-ttu-id="5a372-127">La prima lettera deve essere maiuscola.</span><span class="sxs-lookup"><span data-stu-id="5a372-127">The first letter is required to be uppercase.</span></span> <span data-ttu-id="5a372-128">Gli spazi, i numeri e i caratteri speciali non sono consentiti.</span><span class="sxs-lookup"><span data-stu-id="5a372-128">White space, numbers, and special characters are not allowed.</span></span>

* <span data-ttu-id="5a372-129">`RegularExpression` `Rating` :</span><span class="sxs-lookup"><span data-stu-id="5a372-129">The `RegularExpression` `Rating`:</span></span>

  * <span data-ttu-id="5a372-130">Richiede che il primo carattere sia una lettera maiuscola.</span><span class="sxs-lookup"><span data-stu-id="5a372-130">Requires that the first character be an uppercase letter.</span></span>
  * <span data-ttu-id="5a372-131">Consente i caratteri speciali e i numeri negli spazi successivi.</span><span class="sxs-lookup"><span data-stu-id="5a372-131">Allows special characters and numbers in subsequent spaces.</span></span> <span data-ttu-id="5a372-132">"PG-13" è valido per una classificazione, ma ha esito negativo per un `Genre` .</span><span class="sxs-lookup"><span data-stu-id="5a372-132">"PG-13" is valid for a rating, but fails for a `Genre`.</span></span>

* <span data-ttu-id="5a372-133">L'attributo `[Range]` vincola un valore all'interno di un intervallo specificato.</span><span class="sxs-lookup"><span data-stu-id="5a372-133">The `[Range]` attribute constrains a value to within a specified range.</span></span>
* <span data-ttu-id="5a372-134">L' `[StringLength]` attributo può impostare una lunghezza massima di una proprietà di stringa e, facoltativamente, la lunghezza minima.</span><span class="sxs-lookup"><span data-stu-id="5a372-134">The `[StringLength]` attribute can set a maximum length of a string property, and optionally its minimum length.</span></span>
* <span data-ttu-id="5a372-135">I tipi di valore, ad esempio `decimal` , `int` ,, `float` `DateTime` , sono intrinsecamente necessari e non richiedono l' `[Required]` attributo.</span><span class="sxs-lookup"><span data-stu-id="5a372-135">Value types, such as `decimal`, `int`, `float`, `DateTime`, are inherently required and don't need the `[Required]` attribute.</span></span>

<span data-ttu-id="5a372-136">Le regole di convalida precedenti vengono usate per la dimostrazione, non sono ottimali per un sistema di produzione.</span><span class="sxs-lookup"><span data-stu-id="5a372-136">The preceding validation rules are used for demonstration, they are not optimal for a production system.</span></span> <span data-ttu-id="5a372-137">Il precedente, ad esempio, impedisce l'immissione di un film con solo due caratteri e non consente caratteri speciali in `Genre` .</span><span class="sxs-lookup"><span data-stu-id="5a372-137">For example, the preceding prevents entering a movie with only two chars and doesn't allow special characters in `Genre`.</span></span>

<span data-ttu-id="5a372-138">Le regole di convalida applicate automaticamente da ASP.NET Core consentono di:</span><span class="sxs-lookup"><span data-stu-id="5a372-138">Having validation rules automatically enforced by ASP.NET Core helps:</span></span>
...
### <a name="server-side-validation"></a><span data-ttu-id="5a372-160">Convalida sul lato server</span><span class="sxs-lookup"><span data-stu-id="5a372-160">Server-side validation</span></span>

<span data-ttu-id="5a372-161">Quando JavaScript è disabilitato nel browser, l'invio del modulo con errori verrà inoltrato al server.</span><span class="sxs-lookup"><span data-stu-id="5a372-161">When JavaScript is disabled in the browser, submitting the form with errors will post to the server.</span></span>
...
## <a name="use-datatype-attributes"></a><span data-ttu-id="5a372-183">USA attributi DataType</span><span class="sxs-lookup"><span data-stu-id="5a372-183">Use DataType Attributes</span></span>
...
* <span data-ttu-id="5a372-188">Suggerimenti per il motore di visualizzazione per formattare i dati.</span><span class="sxs-lookup"><span data-stu-id="5a372-188">Hints for the view engine to format the data.</span></span>
* <span data-ttu-id="5a372-189">Fornisce attributi come `<a>` per gli URL e `<a href="mailto:EmailAddress.com">` per la posta elettronica.</span><span class="sxs-lookup"><span data-stu-id="5a372-189">Supplies attributes such as `<a>` for URL's and `<a href="mailto:EmailAddress.com">` for email.</span></span>

<span data-ttu-id="5a372-190">Usare l'attributo `[RegularExpression]` per convalidare il formato dei dati.</span><span class="sxs-lookup"><span data-stu-id="5a372-190">Use the `[RegularExpression]` attribute to validate the format of the data.</span></span> <span data-ttu-id="5a372-191">L'attributo `[DataType]` viene usato per specificare un tipo di dati che è più specifico del tipo intrinseco del database.</span><span class="sxs-lookup"><span data-stu-id="5a372-191">The `[DataType]` attribute is used to specify a data type that's more specific than the database intrinsic type.</span></span> <span data-ttu-id="5a372-192">`[DataType]` gli attributi non sono attributi di convalida.</span><span class="sxs-lookup"><span data-stu-id="5a372-192">`[DataType]` attributes aren't validation attributes.</span></span> <span data-ttu-id="5a372-193">Nell'applicazione di esempio, viene visualizzata solo la data, senza l'ora.</span><span class="sxs-lookup"><span data-stu-id="5a372-193">In the sample application, only the date is displayed, without time.</span></span>

<span data-ttu-id="5a372-194">L' `DataType` enumerazione fornisce molti tipi di dati, ad esempio `Date` ,, `Time` `PhoneNumber` , `Currency` , `EmailAddress` e molto altro.</span><span class="sxs-lookup"><span data-stu-id="5a372-194">The `DataType` enumeration provides many data types, such as `Date`, `Time`, `PhoneNumber`, `Currency`, `EmailAddress`, and more.</span></span> 

<span data-ttu-id="5a372-195">`[DataType]`Attributi:</span><span class="sxs-lookup"><span data-stu-id="5a372-195">The `[DataType]` attributes:</span></span>

* <span data-ttu-id="5a372-196">Può consentire all'applicazione di fornire automaticamente funzionalità specifiche del tipo.</span><span class="sxs-lookup"><span data-stu-id="5a372-196">Can enable the application to automatically provide type-specific features.</span></span> <span data-ttu-id="5a372-197">Ad esempio, è possibile creare un collegamento `mailto:` per `DataType.EmailAddress`.</span><span class="sxs-lookup"><span data-stu-id="5a372-197">For example, a `mailto:` link can be created for `DataType.EmailAddress`.</span></span>
* <span data-ttu-id="5a372-198">Può fornire un selettore `DataType.Date` di data nei browser che supportano HTML5.</span><span class="sxs-lookup"><span data-stu-id="5a372-198">Can provide a date selector `DataType.Date` in browsers that support HTML5.</span></span>
* <span data-ttu-id="5a372-199">Genera HTML 5 `data-` , pronunciato "Dash di dati", attributi utilizzati dai browser HTML 5.</span><span class="sxs-lookup"><span data-stu-id="5a372-199">Emit HTML 5 `data-`, pronounced "data dash", attributes that HTML 5 browsers consume.</span></span>
* <span data-ttu-id="5a372-200">**Non** fornire alcuna convalida.</span><span class="sxs-lookup"><span data-stu-id="5a372-200">Do **not** provide any validation.</span></span>

<span data-ttu-id="5a372-201">`DataType.Date` non specifica il formato della data visualizzata.</span><span class="sxs-lookup"><span data-stu-id="5a372-201">`DataType.Date` doesn't specify the format of the date that's displayed.</span></span> <span data-ttu-id="5a372-202">Per impostazione predefinita, il campo dei dati viene visualizzato in base ai formati predefiniti per il valore `CultureInfo` del server.</span><span class="sxs-lookup"><span data-stu-id="5a372-202">By default, the data field is displayed according to the default formats based on the server's `CultureInfo`.</span></span>

<span data-ttu-id="5a372-203">L'annotazione dei dati `[Column(TypeName = "decimal(18, 2)")]` è necessaria per consentire a Entity Framework Core di eseguire correttamente il mapping di `Price` nella valuta del database.</span><span class="sxs-lookup"><span data-stu-id="5a372-203">The `[Column(TypeName = "decimal(18, 2)")]` data annotation is required so Entity Framework Core can correctly map `Price` to currency in the database.</span></span> <span data-ttu-id="5a372-204">Per altre informazioni, vedere [Tipi di dati](/ef/core/modeling/relational/data-types).</span><span class="sxs-lookup"><span data-stu-id="5a372-204">For more information, see [Data Types](/ef/core/modeling/relational/data-types).</span></span>

<span data-ttu-id="5a372-205">L'attributo `[DisplayFormat]` viene usato per specificare in modo esplicito il formato della data:</span><span class="sxs-lookup"><span data-stu-id="5a372-205">The `[DisplayFormat]` attribute is used to explicitly specify the date format:</span></span>
...
<span data-ttu-id="5a372-206">L' `ApplyFormatInEditMode` impostazione specifica che la formattazione verrà applicata quando il valore viene visualizzato per la modifica.</span><span class="sxs-lookup"><span data-stu-id="5a372-206">The `ApplyFormatInEditMode` setting specifies that the formatting will be applied when the value is displayed for editing.</span></span> <span data-ttu-id="5a372-207">Questo comportamento potrebbe non essere desiderato per alcuni campi.</span><span class="sxs-lookup"><span data-stu-id="5a372-207">That behavior may not be wanted for some fields.</span></span> <span data-ttu-id="5a372-208">Nei valori di valuta, ad esempio, il simbolo di valuta non è in genere desiderato nell'interfaccia utente di modifica.</span><span class="sxs-lookup"><span data-stu-id="5a372-208">For example, in currency values, the currency symbol is usually not wanted in the edit UI.</span></span>

<span data-ttu-id="5a372-209">L'attributo `[DisplayFormat]` può essere usato da solo, ma in genere è consigliabile usare l'attributo `[DataType]`.</span><span class="sxs-lookup"><span data-stu-id="5a372-209">The `[DisplayFormat]` attribute can be used by itself, but it's generally a good idea to use the `[DataType]` attribute.</span></span> <span data-ttu-id="5a372-210">L'attributo `[DataType]` offre la semantica dei dati anziché specificarne il rendering in una schermata.</span><span class="sxs-lookup"><span data-stu-id="5a372-210">The `[DataType]` attribute conveys the semantics of the data as opposed to how to render it on a screen.</span></span> <span data-ttu-id="5a372-211">L' `[DataType]` attributo offre i seguenti vantaggi che non sono disponibili con `[DisplayFormat]` :</span><span class="sxs-lookup"><span data-stu-id="5a372-211">The `[DataType]` attribute provides the following benefits that aren't available with `[DisplayFormat]`:</span></span>

* <span data-ttu-id="5a372-212">Il browser può abilitare le funzionalità HTML5, ad esempio per visualizzare un controllo calendario, il simbolo di valuta appropriato per le impostazioni locali, i collegamenti alla posta elettronica e così via.</span><span class="sxs-lookup"><span data-stu-id="5a372-212">The browser can enable HTML5 features, for example to show a calendar control, the locale-appropriate currency symbol, email links, etc.</span></span>
* <span data-ttu-id="5a372-213">Per impostazione predefinita, il browser esegue il rendering dei dati usando il formato corretto in base alle impostazioni locali.</span><span class="sxs-lookup"><span data-stu-id="5a372-213">By default, the browser renders data using the correct format based on its locale.</span></span>
* <span data-ttu-id="5a372-214">L'attributo `[DataType]` può abilitare il framework di ASP.NET Core a scegliere il modello di campo corretto per il rendering dei dati.</span><span class="sxs-lookup"><span data-stu-id="5a372-214">The `[DataType]` attribute can enable the ASP.NET Core framework to choose the right field template to render the data.</span></span> <span data-ttu-id="5a372-215">`DisplayFormat`, Se usato da solo, usa il modello di stringa.</span><span class="sxs-lookup"><span data-stu-id="5a372-215">The `DisplayFormat`, if used by itself, uses the string template.</span></span>

<span data-ttu-id="5a372-216">**Nota:** la convalida jQuery non funziona con l' `[Range]` attributo e con `DateTime` .</span><span class="sxs-lookup"><span data-stu-id="5a372-216">**Note:** jQuery validation doesn't work with the `[Range]` attribute and `DateTime`.</span></span> <span data-ttu-id="5a372-217">Ad esempio, il codice seguente visualizzerà sempre un errore di convalida sul lato client, anche quando la data è compreso nell'intervallo specificato:</span><span class="sxs-lookup"><span data-stu-id="5a372-217">For example, the following code will always display a client-side validation error, even when the date is in the specified range:</span></span>

```csharp
[Range(typeof(DateTime), "1/1/1966", "1/1/2020")]
   ```

<span data-ttu-id="5a372-218">È consigliabile evitare la compilazione di date rigide nei modelli, quindi l'uso dell' `[Range]` attributo `DateTime` è sconsigliato.</span><span class="sxs-lookup"><span data-stu-id="5a372-218">It's a best practice to avoid compiling hard dates in models, so using the `[Range]` attribute and `DateTime` is discouraged.</span></span> <span data-ttu-id="5a372-219">Usare la [configurazione](xref:fundamentals/configuration/index) per gli intervalli di date e altri valori soggetti a modifiche frequenti anziché specificarli nel codice.</span><span class="sxs-lookup"><span data-stu-id="5a372-219">Use [Configuration](xref:fundamentals/configuration/index) for date ranges and other values that are subject to frequent change rather than specifying it in code.</span></span>
...
# <a name="visual-studio"></a>[<span data-ttu-id="5a372-227">Visual Studio</span><span class="sxs-lookup"><span data-stu-id="5a372-227">Visual Studio</span></span>](#tab/visual-studio)

<span data-ttu-id="5a372-228">La tabella `Movie` ha attualmente lo schema seguente:</span><span class="sxs-lookup"><span data-stu-id="5a372-228">The `Movie` table currently has the following schema:</span></span>
...
<span data-ttu-id="5a372-229">Le modifiche dello schema precedenti non determinano la generazione di un'eccezione da parte di EF.</span><span class="sxs-lookup"><span data-stu-id="5a372-229">The preceding schema changes don't cause EF to throw an exception.</span></span> <span data-ttu-id="5a372-230">Tuttavia, creare una migrazione in modo che lo schema sia coerente con il modello.</span><span class="sxs-lookup"><span data-stu-id="5a372-230">However, create a migration so the schema is consistent with the model.</span></span>

<span data-ttu-id="5a372-231">Dal menu **Strumenti** selezionare **Gestione pacchetti NuGet > Console di Gestione pacchetti**.</span><span class="sxs-lookup"><span data-stu-id="5a372-231">From the **Tools** menu, select **NuGet Package Manager > Package Manager Console**.</span></span>
<span data-ttu-id="5a372-232">Nella Console di Gestione pacchetti immettere i comandi seguenti:</span><span class="sxs-lookup"><span data-stu-id="5a372-232">In the PMC, enter the following commands:</span></span>
...
# <a name="visual-studio-code--visual-studio-for-mac"></a>[<span data-ttu-id="5a372-236">Visual Studio Code / Visual Studio per Mac</span><span class="sxs-lookup"><span data-stu-id="5a372-236">Visual Studio Code / Visual Studio for Mac</span></span>](#tab/visual-studio-code+visual-studio-mac)

<span data-ttu-id="5a372-237">Non sono necessarie migrazioni per SQLite.</span><span class="sxs-lookup"><span data-stu-id="5a372-237">Migrations are not required for SQLite.</span></span>

---

### <a name="publish-to-azure"></a><span data-ttu-id="5a372-238">Pubblicazione in Azure</span><span class="sxs-lookup"><span data-stu-id="5a372-238">Publish to Azure</span></span>
...
> [<span data-ttu-id="5a372-243">Precedente: aggiungere un nuovo campo</span><span class="sxs-lookup"><span data-stu-id="5a372-243">Previous: Add a new field</span></span>](xref:tutorials/razor-pages/new-field)
