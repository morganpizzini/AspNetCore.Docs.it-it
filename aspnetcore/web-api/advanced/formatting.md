...
<span data-ttu-id="eb4f0-189">Nel codice precedente, la chiamata a `AddNewtonsoftJson` Configura le seguenti funzionalità dell'API Web, MVC e :::no-loc(Razor)::: pages da usare `Newtonsoft.Json` :</span><span class="sxs-lookup"><span data-stu-id="eb4f0-189">In the preceding code, the call to `AddNewtonsoftJson` configures the following Web API, MVC, and :::no-loc(Razor)::: Pages features to use `Newtonsoft.Json`:</span></span>
