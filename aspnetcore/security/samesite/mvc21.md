...
<span data-ttu-id="b2c7c-108">Di seguito è riportato un esempio di come scrivere un attributo navigava sullostesso sito in un oggetto cookie :</span><span class="sxs-lookup"><span data-stu-id="b2c7c-108">Following is an example of how to write a SameSite attribute on a cookie:</span></span>
...
## <a name="setting-no-loccookie-authentication-and-session-state-no-loccookies"></a><span data-ttu-id="b2c7c-109">Impostazione dell' Cookie autenticazione e dello stato della sessione cookie</span><span class="sxs-lookup"><span data-stu-id="b2c7c-109">Setting Cookie Authentication and Session State cookies</span></span>

<span data-ttu-id="b2c7c-110">Cookie l'autenticazione, lo stato della sessione e [vari altri componenti](../samesite.md?view=aspnetcore-2.1) impostano le opzioni di navigava sullostesso sito tramite Cookie Opzioni, ad esempio</span><span class="sxs-lookup"><span data-stu-id="b2c7c-110">Cookie authentication, session state and [various other components](../samesite.md?view=aspnetcore-2.1) set their sameSite options via Cookie options, for example</span></span>
...
<span data-ttu-id="b2c7c-113">Se si esegue il [progetto di esempio](https://github.com/blowdart/AspNetSameSiteSamples/tree/master/AspNetCore21MVC), caricare il debugger del browser nella pagina iniziale e utilizzarlo per visualizzare la cookie raccolta per il sito.</span><span class="sxs-lookup"><span data-stu-id="b2c7c-113">If you run the [sample project](https://github.com/blowdart/AspNetSameSiteSamples/tree/master/AspNetCore21MVC), load your browser debugger on the initial page and use it to view the cookie collection for the site.</span></span> <span data-ttu-id="b2c7c-114">A tale scopo, in Edge e Chrome premere `F12` quindi selezionare la `Application` scheda e fare clic sull'URL del sito sotto l' `Cookies` opzione nella `Storage` sezione.</span><span class="sxs-lookup"><span data-stu-id="b2c7c-114">To do so in Edge and Chrome press `F12` then select the `Application` tab and click the site URL under the `Cookies` option in the `Storage` section.</span></span>
...
## <a name="intercepting-no-loccookies"></a><a name="interception"></a><span data-ttu-id="b2c7c-117">Intercettazione di cookie s</span><span class="sxs-lookup"><span data-stu-id="b2c7c-117">Intercepting cookies</span></span>

<span data-ttu-id="b2c7c-118">Per intercettare i cookie , per modificare il valore None in base al relativo supporto nell'agente browser dell'utente, è necessario usare il `CookiePolicy` middleware.</span><span class="sxs-lookup"><span data-stu-id="b2c7c-118">In order to intercept cookies, to adjust the none value according to its support in the user's browser agent you must use the `CookiePolicy` middleware.</span></span> <span data-ttu-id="b2c7c-119">Questa deve essere inserita nella pipeline di richieste HTTP **prima** di tutti i componenti che scrivono cookie e configurati in `ConfigureServices()` .</span><span class="sxs-lookup"><span data-stu-id="b2c7c-119">This must be placed into the http request pipeline **before** any components that write cookies and configured within `ConfigureServices()`.</span></span>

<span data-ttu-id="b2c7c-120">Per inserirlo nell'uso della pipeline `app.UseCookiePolicy()` nel `Configure(IApplicationBuilder, IHostingEnvironment)` metodo in [Startup.cs](https://github.com/blowdart/AspNetSameSiteSamples/blob/master/AspNetCore21MVC/Startup.cs).</span><span class="sxs-lookup"><span data-stu-id="b2c7c-120">To insert it into the pipeline use `app.UseCookiePolicy()` in the `Configure(IApplicationBuilder, IHostingEnvironment)` method in [Startup.cs](https://github.com/blowdart/AspNetSameSiteSamples/blob/master/AspNetCore21MVC/Startup.cs).</span></span> <span data-ttu-id="b2c7c-121">Ad esempio:</span><span class="sxs-lookup"><span data-stu-id="b2c7c-121">For example:</span></span>
...
<span data-ttu-id="b2c7c-122">Nel `ConfigureServices(IServiceCollection services)` configurare il cookie criterio per la chiamata a una classe helper quando cookie vengono aggiunti o eliminati.</span><span class="sxs-lookup"><span data-stu-id="b2c7c-122">Then in the `ConfigureServices(IServiceCollection services)` configure the cookie policy to call out to a helper class when cookies are appended or deleted.</span></span> <span data-ttu-id="b2c7c-123">Ad esempio:</span><span class="sxs-lookup"><span data-stu-id="b2c7c-123">For example:</span></span>
...
<span data-ttu-id="b2c7c-124">Funzione helper `CheckSameSite(HttpContext, CookieOptions)` :</span><span class="sxs-lookup"><span data-stu-id="b2c7c-124">The helper function `CheckSameSite(HttpContext, CookieOptions)`:</span></span>

* <span data-ttu-id="b2c7c-125">Viene chiamato quando cookie vengono aggiunti alla richiesta o eliminati dalla richiesta.</span><span class="sxs-lookup"><span data-stu-id="b2c7c-125">Is called when cookies are appended to the request or deleted from the request.</span></span>
