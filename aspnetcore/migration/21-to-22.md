...
Per adottare il [modello di hosting in-process per IIS](xref:host-and-deploy/iis/index#in-process-hosting-model), aggiungere `<AspNetCoreHostingModel>` la proprietà con un valore `InProcess` di a `<PropertyGroup>` nel file di progetto:
...
* `<handlers>` Nella voce che aggiunge il modulo ASP.NET Core (`name="aspNetCore"`), modificare il valore `modules` dell'attributo da `AspNetCoreModule` a `AspNetCoreModuleV2`.
...
Se l'app non chiama `CreateDefaultBuilder` `Program` e compila manualmente l'host nella classe, chiamare <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> **prima** di chiamare `ConfigureKestrel`:
