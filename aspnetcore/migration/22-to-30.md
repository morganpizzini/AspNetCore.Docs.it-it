...
Questo articolo illustra come aggiornare un progetto di ASP.NET Core 2,2 esistente a ASP.NET Core 3,0. Potrebbe essere utile creare un nuovo progetto di ASP.NET Core 3,0 per:

* Confrontare con il codice di ASP.NET Core 2,2.
* Copiare le modifiche rilevanti nel progetto ASP.NET Core 3,0.
...
Se la soluzione si basa su un file [global.js](/dotnet/core/tools/global-json) per fare riferimento a una versione specifica del .NET Core SDK, aggiornare la relativa `version` proprietà alla versione 3,0 installata nel computer:
...
* Nel `<PropertyGroup>` eseguire le operazioni seguenti:
  ...
  * Rimuove l' `<AspNetCoreHostingModel>` elemento. Per ulteriori informazioni, vedere [modello di hosting in-process](#in-process-hosting-model) in questo documento.

* Nel `<ItemGroup>` eseguire le operazioni seguenti:
  * `Microsoft.AspNetCore.App` viene rimosso. Per ulteriori informazioni, vedere [riferimento a Framework](#framework-reference) in questo documento.
...
<details>
    ...
    <ul>
        ...
        <li>Microsoft. AspNetCore. Authentication. Cookie s</li>
        <li>Microsoft.AspNetCore.Authentication.Core</li>
        <li>Microsoft.AspNetCore.Authentication.OAuth</li>
        <li>Microsoft.AspNetCore.Authorization.Policy</li>
        <li>Microsoft. AspNetCore. Cookie Politica</li>
        ...
        <li>Microsoft. AspNetCore. Mvc. Razor Pagine</li>
        <li>Microsoft.AspNetCore.Mvc.TagHelpers</li>
        <li>Microsoft.AspNetCore.Mvc.ViewFeatures</li>
        <li>Microsoft. AspNetCore.Razor</li>
        <li>Microsoft Razor . AspNetCore. Runtime</li>
        <li>Microsoft Razor . AspNetCore. Design</li>
        ...
        <li>Microsoft SignalR . AspNetCore. Core</li>
...
Le funzionalità di ASP.NET Core disponibili tramite uno dei pacchetti elencati in precedenza sono disponibili come parte del `Microsoft.AspNetCore.App` Framework condiviso. Il *Framework condiviso* è il set di assembly (file con *estensione dll* ) installato nel computer e include un componente di runtime e un Targeting Pack. Per altre informazioni, vedere [The shared framework](https://natemcmaster.com/blog/2018/08/29/netcore-primitives-2/) (Il framework condiviso).
...
* I progetti destinati `Microsoft.NET.Sdk` `Microsoft.NET.Sdk.Razor` a o SDK devono aggiungere un esplicito `FrameworkReference` a `Microsoft.AspNetCore.App` :
...
Per determinare se l'app ha una dipendenza (diretta o indiretta) nel Framework condiviso ASP.NET Core, esaminare i *runtimeconfig.jssul* file generato durante una compilazione/pubblicazione dell'app. Il file JSON seguente mostra una dipendenza dal Framework condiviso ASP.NET Core:
...
ASP.NET Core 3,0 rimuove alcuni assembly che in precedenza facevano parte del `Microsoft.AspNetCore.App` riferimento al pacchetto. Per visualizzare gli assembly rimossi, confrontare le due cartelle del Framework condivise. Ad esempio, un confronto tra le versioni 2.2.7 e 3.0.0:
...
* Interfaccia utente di Identity

  È possibile aggiungere il supporto per l' [ Identity interfaccia utente](xref:security/authentication/identity) facendo riferimento a [Microsoft. AspNetCore. Identity Pacchetto dell'interfaccia utente](https://www.nuget.org/packages/Microsoft.AspNetCore.Identity.UI) .
...
* Formattazione e supporto per la negoziazione `System.Net.HttpClient` del contenuto per: il pacchetto NuGet [Microsoft. AspNet. WebAPI. client](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Client/) fornisce un'estendibilità utile a `System.Net.HttpClient` con API quali `ReadAsAsync` e `PostJsonAsync` .

* Razor compilazione Runtime: il supporto per la compilazione in fase di esecuzione di Razor viste e pagine fa ora parte di [Microsoft. AspNetCore. Mvc. Razor . RuntimeCompilation](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.Razor.RuntimeCompilation).

* `Newtonsoft.Json`Supporto di MVC (JSON.NET): il supporto per l'uso di MVC con `Newtonsoft.Json` fa ora parte di [`Microsoft.AspNetCore.Mvc.NewtonsoftJson`](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson) .

## <a name="startup-changes"></a>Modifiche all'avvio

La figura seguente mostra le righe eliminate e modificate in un'app Web di ASP.NET Core pagine di 2,2 Razor :

![righe eliminate e modificate in un ASP.NET Core 2,2::: NO-LOC (Razor)::: Web App](22-to-30/_static/startup2.2.png)

Nell'immagine precedente, il codice eliminato viene visualizzato in rosso. Il codice eliminato non Mostra il cookie codice delle opzioni, che è stato eliminato prima di confrontare i file.

La figura seguente mostra le righe aggiunte e modificate in un'app Web di ASP.NET Core pagine di 3,0 Razor :
...
* `IHostingEnvironment` per `IWebHostEnvironment` , vedere [questo annuncio su GitHub](https://github.com/dotnet/AspNetCore/issues/7749).
...
* `app.UseEndpoints`, vedere [ Razor pagine](#razor-pages) o [eseguire la migrazione di Startup.ConfigUre](#migrate-startupconfigure) in questo documento.
...
### <a name="no-locrazor-class-library"></a>Razor Libreria di classi
...
Per impostazione predefinita, i progetti sono [basati sul modello di hosting in-process](xref:host-and-deploy/aspnet-core-module#in-process-hosting-model) in ASP.NET Core 3,0 o versione successiva. Se il valore è, è possibile rimuovere facoltativamente la `<AspNetCoreHostingModel>` proprietà nel file di progetto `InProcess` .

## <a name="kestrel"></a>Kestrel

### <a name="configuration"></a>Configurazione

Eseguire la migrazione della configurazione di Gheppio al [generatore host Web](#hostb) fornito da `ConfigureWebHostDefaults` (*Program.cs*):
...
Se l'applicazione crea manualmente l'host con `ConfigureWebHost` anziché `ConfigureWebHostDefaults` , chiamare `UseKestrel` sul generatore host Web:
...
Per altre informazioni, vedere [l'esempio TlsFilterConnectionHandler nella sezione ListenOptions. Protocols dell'articolo di Gheppio](../fundamentals/servers/kestrel.md?view=aspnetcore-3.0#listenoptionsprotocols).
...
Per la versione 3,0 `RequestTrailerExtensions` sono disponibili i metodi seguenti:
...
* `CheckTrailersAvailable`: Controlla se la richiesta supporta i trailer e se è disponibile per la lettura. Questo controllo non presuppone che siano presenti trailer da leggere. Non è possibile leggere i trailer anche se `true` viene restituito da questo metodo.
* `GetTrailer`: Ottiene l'intestazione finale richiesta dalla risposta. Verificare `SupportsTrailers` prima di chiamare `GetTrailer` oppure <xref:System.NotSupportedException> può verificarsi se la richiesta non supporta le intestazioni finali.
...
Se si riscontrano problemi con <xref:System.IO.TextWriter> le implementazioni o altri flussi che chiamano API sincrone in [Dispose](/dotnet/standard/garbage-collection/implementing-dispose), chiamare invece la nuova <xref:System.IO.Stream.DisposeAsync*> API.
...
### <a name="output-formatter-buffering"></a>Buffering del formattatore di output

[Newtonsoft.Jssui](https://www.newtonsoft.com/json) <xref:System.Xml.Serialization.XmlSerializer> <xref:System.Runtime.Serialization.DataContractSerializer> formattatori di output basati su, e supportano solo la serializzazione sincrona. Per consentire a questi formattatori di funzionare con le restrizioni [AllowSynchronousIO](https://github.com/dotnet/aspnetcore/issues/7644) del server, MVC memorizza nel buffer l'output di questi formattatori prima della scrittura su disco. In seguito al buffering, MVC includerà l'intestazione Content-Length quando risponderà usando questi formattatori.

<xref:System.Text.Json> supporta la serializzazione asincrona e di conseguenza il `System.Text.Json` formattatore basato non memorizza nel buffer. Prendere in considerazione l'uso di questo formattatore per migliorare le prestazioni.

Per disabilitare il buffering, le applicazioni possono configurare <xref:Microsoft.AspNetCore.Mvc.MvcOptions.SuppressOutputFormatterBuffering> all'avvio:

```csharp
services.AddControllers(options => options.SuppressOutputFormatterBuffering = true)
```

Si noti che questo può comportare l'applicazione che genera un'eccezione in fase di esecuzione se `AllowSynchronousIO` non è anche configurato.
...
In ASP.NET Core 2,1 il contenuto delle *Microsoft.AspNetCore.Server.Kestrel.Https.dll* è stato spostato in *Microsoft.AspNetCore.Server.Kestrel.Core.dll*. Si tratta di un aggiornamento senza interruzioni che utilizza `TypeForwardedTo` gli attributi. Per 3,0, l'assembly *Microsoft.AspNetCore.Server.Kestrel.Https.dll* vuoto e il pacchetto NuGet sono stati rimossi.
...
Come parte del lavoro per [migliorare il Framework condiviso ASP.NET Core](https://blogs.msdn.microsoft.com/webdev/2018/10/29/a-first-look-at-changes-coming-in-asp-net-core-3-0/), [Newtonsoft.Json (JSON.NET)](https://www.newtonsoft.com/json/help/html/Introduction.htm) è stato rimosso dal framework ASP.NET Core Shared.

Il serializzatore JSON predefinito per ASP.NET Core ora è <xref:System.Text.Json> nuovo in .NET Core 3,0. Si consiglia di utilizzare `System.Text.Json` quando possibile. Si tratta di prestazioni elevate e non richiede una dipendenza della libreria aggiuntiva. Tuttavia, poiché `System.Text.Json` è nuovo, è possibile che al momento non siano presenti funzionalità richieste dall'app. Per ulteriori informazioni, vedere [How to migrate from Newtonsoft.Json to System.Text.Json](/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to).

### <a name="use-newtonsoftjson-in-an-aspnet-core-30-no-locsignalr-project"></a>Usare Newtonsoft.Jsin un progetto ASP.NET Core 3,0 SignalR
...
* Nel client, concatenare una `AddNewtonsoftJsonProtocol` chiamata al metodo all' `HubConnectionBuilder` istanza:
...
* Nel server, concatenare una `AddNewtonsoftJsonProtocol` chiamata al metodo alla `AddSignalR` chiamata al metodo in `Startup.ConfigureServices` :
...
### <a name="use-newtonsoftjson-in-an-aspnet-core-30-mvc-project"></a>Usare Newtonsoft.Jsin un progetto MVC ASP.NET Core 3,0

* Installare il [`Microsoft.AspNetCore.Mvc.NewtonsoftJson`](https://nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson) pacchetto.
...
* Aggiornamento `Startup.ConfigureServices` da chiamare `AddNewtonsoftJson` .
  ...
  **Nota:** Se il `AddNewtonsoftJson` metodo non è disponibile, assicurarsi di aver installato il [`Microsoft.AspNetCore.Mvc.NewtonsoftJson`](https://nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson) pacchetto. Un errore comune consiste nell'installare il [Newtonsoft.Jsnel](https://www.nuget.org/packages/Newtonsoft.Json/) pacchetto anziché nel [`Microsoft.AspNetCore.Mvc.NewtonsoftJson`](https://nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson) pacchetto.

Per altre informazioni, vedere [aggiungere Newtonsoft.Jssupporto in formato JSON basato su](xref:web-api/advanced/formatting#add-newtonsoftjson-based-json-format-support).
...
Nell'esempio seguente viene aggiunto il supporto per le Razor pagine e il supporto minimo del controller. Il modello di applicazione Web usa questo codice:
...
Il routing degli endpoint supporta la stessa sintassi del modello di route e le funzionalità di creazione di modelli di route di `IRouter` . Il routing degli endpoint supporta `IRouteConstraint` . Il routing degli endpoint supporta `[Route]` , `[HttpGet]` e gli altri attributi di routing MVC.

Per la maggior parte delle applicazioni, è `Startup` necessario apportare solo modifiche.
...
* Sostituire `IHostingEnvironment` con `IWebHostEnvironment` e aggiungere un' `using` istruzione per lo <xref:Microsoft.Extensions.Hosting?displayProperty=fullName> spazio dei nomi.
...
Nelle versioni precedenti di ASP.NET Core, il supporto dell'autorizzazione è stato fornito tramite l' `[Authorize]` attributo. Il middleware di autorizzazione non era disponibile. In ASP.NET Core 3,0 è necessario il middleware di autorizzazione. Si consiglia di inserire il middleware di autorizzazione ASP.NET Core ( `UseAuthorization` ) subito dopo `UseAuthentication` . Il middleware di autorizzazione può essere configurato anche con criteri predefiniti, che possono essere sottoposti a override.

In ASP.NET Core 3,0 o versione successiva, `UseAuthorization` viene chiamato in `Startup.Configure` e per quanto segue è `HomeController` necessario un utente connesso:
...
Quando si usa il routing degli endpoint, è consigliabile evitare di configurare `<xref:Microsoft.AspNetCore.Mvc.Authorization.AuthorizeFilter>` e invece di basarsi sul middleware di autorizzazione.  Se l'app usa un `AuthorizeFilter` come filtro globale in MVC, è consigliabile effettuare il refactoring del codice per fornire un criterio nella chiamata a `AddAuthorization` .

`DefaultPolicy`Inizialmente è configurato per richiedere l'autenticazione, pertanto non è necessaria alcuna configurazione aggiuntiva. Nell'esempio seguente gli endpoint MVC sono contrassegnati come in `RequireAuthorization` modo che tutte le richieste devono essere autorizzate in base a `DefaultPolicy` . Tuttavia, `HomeController` consente l'accesso senza che l'utente acceda all'app a causa di `[AllowAnonymous]` :
...
L'autorizzazione può essere configurata anche per classi specifiche di endpoint. Il codice seguente è un esempio di conversione di un'app MVC che ha configurato una globale `AuthorizeFilter` in un'app con criteri specifici che richiedono l'autorizzazione:
...
In alternativa, tutti gli endpoint possono essere configurati in modo da richiedere l'autorizzazione senza `[Authorize]` o `RequireAuthorization` configurando un `FallbackPolicy` . `FallbackPolicy`È diverso da `DefaultPolicy` . Il `DefaultPolicy` viene attivato da `[Authorize]` o `RequireAuthorization` , mentre l'oggetto `FallbackPolicy` viene attivato quando non è impostato alcun altro criterio. `FallbackPolicy` Inizialmente è configurato per consentire le richieste senza autorizzazione.

L'esempio seguente è identico a quello dell' `DefaultPolicy` esempio precedente, ma usa `FallbackPolicy` per richiedere sempre l'autenticazione su tutti gli endpoint tranne quando `[AllowAnonymous]` è specificato:
...
Ogni endpoint può inoltre personalizzare i requisiti di autorizzazione. Nell'esempio seguente, `UseAuthorization` elabora l'autorizzazione con `DefaultPolicy` , ma l'endpoint di `/healthz` controllo integrità richiede un `admin` utente:
...
Il mapping dei controller si verifica ora all'interno di `UseEndpoints` .
...
In ASP.NET Core 3,0 ASP.NET Core MVC rimuove il `Async` suffisso dai nomi delle azioni del controller. Questo nuovo valore predefinito è influenzato dalla generazione del routing e del collegamento. Ad esempio:
...
* Generazione del collegamento richiesta specifica del `Async` suffisso. Ad esempio:
...
* Per la generazione di collegamenti non è necessario specificare il `Async` suffisso. Ad esempio:

    ```cshtml
    <a asp-controller="Products" asp-action="List">List</a>
    ```

Questa modifica non influisce sui nomi specificati usando l' [`[ActionName]`](/dotnet/api/microsoft.aspnetcore.mvc.actionnameattribute) attributo. Il comportamento predefinito può essere disabilitato con il codice seguente in `Startup.ConfigureServices` :
...
Come illustrato nella documentazione relativa alle [differenze rispetto alle versioni precedenti del routing](xref:fundamentals/routing#differences-from-earlier-versions-of-routing), esistono alcune differenze nella generazione dei collegamenti ( `Url.Link` ad esempio, usando e API simili). Queste includono:

* Per impostazione predefinita, quando si usa il routing degli endpoint, la combinazione dei parametri di route negli URI generati non viene necessariamente mantenuta. Questo comportamento può essere controllato con l' `IOutboundParameterTransformer` interfaccia.
...
Le Razor pagine di mapping avvengono ora all'interno di `UseEndpoints` .

Aggiungere `MapRazorPages` se l'app usa le Razor pagine. Poiché il routing degli endpoint include il supporto per molti Framework, l'aggiunta di Razor pagine è ora il consenso esplicito.

Nel metodo seguente `Startup.Configure` , `MapRazorPages` aggiunge il supporto per le Razor pagine:
...
Nell'esempio seguente `MapHealthChecks` viene chiamato per un endpoint di controllo integrità in `/healthz` :
...
I modelli di ASP.NET Core 3,0 usano un [host generico](xref:fundamentals/host/generic-host). Le versioni precedenti usavano il [Web host](xref:fundamentals/host/web-host). Il codice seguente illustra la classe generata dal modello di ASP.NET Core 3,0 `Program` :

[!code-csharp[](22-to-30/samples/Program.cs?name=snippet)]

Il codice seguente illustra la classe generata dal modello di ASP.NET Core 2,2 `Program` :
...
La modifica più significativa da `WebHostBuilder` a `HostBuilder` è nell' [inserimento delle dipendenze (di)](xref:fundamentals/dependency-injection). Quando `HostBuilder` si usa, è possibile inserire solo il codice seguente nel `Startup` costruttore:

* <xref:Microsoft.Extensions.Configuration.IConfiguration>
* <xref:Microsoft.Extensions.Hosting.IHostEnvironment>
* <xref:Microsoft.AspNetCore.Hosting.IWebHostEnvironment>

`HostBuilder`Vincoli di di:
...
Il ASP.NET Core 2,2 e i `AddAuthorization` Metodi inferiori *Microsoft.AspNetCore.Authorization.dll*:
...
## <a name="no-locidentity-ui"></a>Interfaccia utente di Identity

Identity Aggiornamenti dell'interfaccia utente per ASP.NET Core 3,0:

* Aggiungere un riferimento al pacchetto a [Microsoft. AspNetCore. Identity Interfaccia utente](https://nuget.org/packages/Microsoft.AspNetCore.Identity.UI).
* Le app che non usano le Razor pagine devono chiamare `MapRazorPages` . Vedere le [ Razor pagine](#razor-pages) di questo documento.
* Bootstrap 4 è il Framework dell'interfaccia utente predefinito. Impostare una `IdentityUIFrameworkVersion` proprietà del progetto per modificare il valore predefinito. Per altre informazioni, vedere [questo annuncio su GitHub](https://github.com/aspnet/Announcements/issues/380).

## SignalR

Il SignalR client JavaScript è stato modificato da `@aspnet/signalr` a `@microsoft/signalr` . Per rispondere a questa modifica, modificare i riferimenti in *package.jssu* file, `require` istruzioni e istruzioni ECMAScript `import` .

### <a name="systemtextjson-is-the-default-protocol"></a>System.Text.Jssu è il protocollo predefinito
...
1. Aggiornamento `Startup.ConfigureServices` da chiamare `AddRazorRuntimeCompilation` :
    ...
    Per ASP.NET Core Razor pagine, usare il codice seguente:
...
L'esempio in https://github.com/aspnet/samples/tree/master/samples/aspnetcore/mvc/runtimecompilation Mostra un esempio di abilitazione condizionale della compilazione di runtime negli ambienti di sviluppo.

Per ulteriori informazioni sulla Razor compilazione di file, vedere <xref:mvc/views/view-compilation> .
...
Il sistema di convalida in .NET Core 3,0 e versioni successive considera i parametri non nullable o le proprietà associata come se disponesse di un `[Required]` attributo. Per ulteriori informazioni, vedere l' [attributo [Required]](xref:mvc/models/validation?view=aspnetcore-3.1#required-attribute).
...
Per le app che usano <xref:Microsoft.AspNetCore.TestHost.TestServer> direttamente con l' [host generico](xref:fundamentals/host/web-host), creare in `TestServer` un <xref:Microsoft.AspNetCore.Hosting.IWebHostBuilder> in <xref:Microsoft.Extensions.Hosting.GenericHostWebHostBuilderExtensions.ConfigureWebHost%2A> :
...
## <a name="endpoint-routing-with-catch-all-parameter"></a>Routing dell'endpoint con parametro catch-all
