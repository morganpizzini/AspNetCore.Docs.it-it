...
1. In `wwwroot/index.html`:
   ...
   * Aggiungere un `<link>` elemento all' `<head>` elemento per gli stili con ambito.
   * Modificare l'elemento che carica il `App` componente.
...
1. In `Shared/MainLayout.razor` racchiudere tutto il contenuto di un `<div>` elemento con il valore `id` di `page` :
...
1. Il file di base più recente `wwwroot/css/app.css` include gli stili seguenti. Rimuovere gli stili aggiuntivi lasciando gli stili seguenti e quelli aggiunti all'app.

    `wwwroot/css/app.css` (stili di base non inclusi gli stili aggiunti dallo sviluppatore):
