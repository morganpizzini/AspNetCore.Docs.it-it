...
Identity2,1 espone gli endpoint con l' `Identity` area. La tabella seguente, ad esempio, Mostra esempi di Identity endpoint che variano da 2,0 a 2,1:
...
* Eliminare il Identity codice 2,0 esistente e [l' Identity impalcatura](xref:security/authentication/scaffold-identity) nel progetto. Il progetto utilizzerà la [ Razor libreria di classi](xref:razor-pages/ui-class) [ASP.NET Core Identity ](xref:security/authentication/identity) . È possibile generare codice e interfaccia utente per qualsiasi Identity codice dell'interfaccia utente modificato. Applicare le modifiche del codice al nuovo codice dell'interfaccia utente con impalcature.
...
### <a name="replace-identity-20-ui-with-the-identity-21-razor-class-library"></a>Sostituire Identity l'interfaccia utente di 2,0 con la Identity Razor libreria di classi 2,1

Questa sezione descrive i passaggi per sostituire il codice generato dal modello di ASP.NET Core 2,0 Identity con la [ Razor libreria di classi](xref:razor-pages/ui-class) [ASP.NET Core Identity ](xref:security/authentication/identity) . I passaggi seguenti si riferiscono a un Razor progetto di pagine, ma l'approccio per un progetto MVC è simile.
...
## <a name="changes-to-razor-pages-projects-razor-files"></a>Modifiche ai Razor file di progetti di pagine Razor
...
## <a name="changes-to-mvc-projects-razor-files"></a>Modifiche ai file di progetti MVC Razor
